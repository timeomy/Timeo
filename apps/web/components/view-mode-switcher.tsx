...
type RoleOption = "admin" | "staff" | "coach" | "customer";
...
function allowedRolesForTenant(
  ...
  if (isPlatformAdmin) return ["admin", "staff", "coach", "customer"];

  const normalized = normalizeTimeoRole(tenantRole);
  if (normalized === "admin") return ["admin", "staff", "coach", "customer"];
  if (normalized === "staff") return ["staff", "customer"];
  if (normalized === "coach") return ["coach", "customer"];
  return ["customer"];
}
...
export function ViewModeSwitcher() {
  ...
  const currentLabel = useMemo(() => {
    ...
    const effectiveRole = normalizeTimeoRole(viewAsRole ?? activeTenant?.role ?? "customer");
    const roleLabel =
      effectiveRole === "admin"
        ? "Admin"
        : effectiveRole === "coach"
